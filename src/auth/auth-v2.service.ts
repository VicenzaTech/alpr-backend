...
export class AuthV2Service {
    ...
    async registerWithFirebase(dto: FirebaseRegisterDto) {
        try {
            ...
            if (dto.role) {
                await this.firebaseAuthService.setCustomUserClaims(firebaseUser.uid, { role: dto.role });
            }

            const user = await this.upsertUserFromFirebase(firebaseUser.uid, firebaseUser.email ?? dto.email, {
                fullName: dto.fullName,
                phone: dto.phone,
                avatarUrl: dto.avatarUrl,
            ...
            return {
                ...
                firebase: {
                    ...
                    customToken: await this.firebaseAuthService.generateCustomToken(firebaseUser.uid, {
                        role: user.role,
                    }),
                },
            };
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unable to register user with Firebase';
            this.logger.error(`Failed to register Firebase user: ${message}`, error instanceof Error ? error.stack : undefined);
    ...
    async loginWithFirebase(dto: FirebaseAuthDto) {
        ...
        await this.firebaseAuthService.setCustomUserClaims(decoded.uid, customClaims);
        const customToken = await this.firebaseAuthService.generateCustomToken(decoded.uid, customClaims);
    ...
    private async upsertUserFromFirebase(uid: string, email: string, overrides: ProfileOverrides): Promise<User> {
