  ...
  async validateOAuthUser(profile: OAuthProfile): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      user = this.userRepo.create({
        ...
        email: normalizedEmail,
        fullName:
          profile.fullName ?? this.buildUsernameFromEmail(normalizedEmail),
      ...
      this.logger.debug(
        `Created OAuth user via ${profile.provider} (${normalizedEmail})`,
      );
    } else {
      ...
      this.logger.debug(
        `Updated OAuth user via ${profile.provider} (${normalizedEmail})`,
      );
    }
