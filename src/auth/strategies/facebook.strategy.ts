...
export class FacebookStrategy extends PassportStrategy(Strategy, 'facebook') {
    ...
    constructor(
        ...
        const callbackURL = configService.get<string>('FACEBOOK_CALLBACK_URL') ?? 'http://localhost:5555/api/v3/auth/facebook/callback';
        ...
        if (!this.isConfigured) {
            this.logger.warn('Facebook OAuth credentials missing. Facebook login endpoints will be disabled.');
        }
    }

    async validate(_accessToken: string, _refreshToken: string, profile: Profile, done: (err: Error | null, user?: any, info?: any) => void) {
        if (!this.isConfigured) {
