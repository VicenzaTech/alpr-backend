...
export class CameraService {
    ...
    async create(createCameraDto: CreateCameraDto): Promise<cameraResponseType> {
        ...
        if (foundCamera) throw new BadRequestException(`Camera với code ${createCameraDto.code} đã tồn tại!`)
    ...
    async activate(id: number) {
        const foundCamera = await this.ensureExistCameraById(id);
        if (foundCamera.isActive === true) throw new BadRequestException('Camera vẫn đang hoạt động trước đó')
    ...
    async inActivate(id: number) {
        const foundCamera = await this.ensureExistCameraById(id);
        if (foundCamera.isActive === false) throw new BadRequestException('Camera đã ngừng hoạt động trước đó')
