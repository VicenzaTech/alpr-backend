...
import { Customer, CustomerStatus, CustomerType } from 'src/database/entities/master-data.entity';
...
const CUSTOMER_SORTABLE_FIELDS = ['code', 'name', 'createdAt', 'updatedAt'] as const;
...
export class CustomerService {
    ...
    async findAll(params: CustomerListParams): Promise<PaginatedResult<CustomerResponse>> {
        const {
            page = 1,
            limit = 20,
            sort,
    ...
    async update(customerId: number, dto: UpdateCustomerDto): Promise<CustomerResponse> {
    ...
    private async ensureCustomerExists(customerId: number, withRelations = false): Promise<Customer> {
